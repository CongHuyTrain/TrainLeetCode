...
//    n for which this array could be a permutation of base[n], is n = 3. However, base[3] has four 
//    elements but array nums has three.Therefore, it can not be a permutation of base[3] = [1, 2, 3, 3].So the answer is false.
//    Example 2 :
//
//    Input : nums = [1, 3, 3, 2]
//    Output : true
...
//    which this array could be a permutation of base[n], is n = 1. It can be seen that nums
//    is a permutation of base[1] = [1, 1].Therefore, the answer is true.
//    Example 4 :
...
//    However, base[4] has five elements but array nums has six.Therefore, it can not be a
...
public:
    bool isGood(vector<int>& nums) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            count[nums[i]]++;
            if (count[nums[i]] >= 2)    if (nums[i] != n - 1) return false;
        }
